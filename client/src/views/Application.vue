<template>
  <div class="maximize home">

    <HeaderNav :loading="loading" v-on:indexChange="indexChange"/>
    <Settings :scope="'Request'"/>
    <Settings :scope="'Measurement'"/>
    ...
    <b-row class="max-height">
      ...
      <!-- This is the center area of the application page, where the graphs are drawn -->
      <b-col>
        <b-tabs class="maximize" v-model="activeTab">

          <!-- ToF-E histogram tab -->
          <b-tab class="maximize" title="Tof-E histogram" lazy>
            <TofeHistogram v-if="tofeData.length"
                           :ticks="ticks"
              ...
              <div class="column">
                ...
                <label class="settings-row">X axis units
                  <select v-model="xUnits">
                    ...
                    <option>nm</option>
                    <!-- TODO: reference density option for nm scale, in measurement.profile -->
                  </select>
                </label>
          ...
          <b-modal ref="energy-settings" id="energy-settings"
            ...
            <div v-else >
              ...
              <label class="settings-row">Histogram bin width:
                <input type="number" step="0.001" v-model="spectraBinWidth">
              </label>
            </div>
          </b-modal>
...
<script>
  ...
  import TofeHistogram from '../components/Graphs/TofeHistogram.vue';
  import DepthProfiles from '../components/Graphs/DepthProfiles.vue';
  import EnergySpectra from '../components/Graphs/EnergySpectra.vue';
  import ElementLosses from '../components/Graphs/ElementLosses.vue';
  import LeftNav from '../components/LeftNav.vue';
  import HeaderNav from '../components/HeaderNav.vue';
  import Modals from '../components/Modals.vue';
  import Settings from '../components/Modals/Settings.vue';

  // TODO: finish refactoring each graph to its own component
  export default {
    name: 'Application',
    ...
    methods: {
      ...
      getEnergySpectra() {
        ...
        axios.get('http://localhost:5000/energy_spectra', { params: energySettings })
          .then((res) => {
       ...
       */
      getDepth() {
        if (this.availableCuts.length === 0) return;
        ...
        axios.get('http://localhost:5000/depth_profiles', { params: depthSettings })
          .then((res) => {
            this.depthGraph(res);
          })
          .catch((error) => {
      ...
      /**
       * Fetch and update depth profile data.
       */
      depthGraph(res) {
      ...
      /**
       * Fetch data for ToF-E histogram, including user selections.
       */
      getTofeData() {
        // user input is saved as a string
        let compressionX = parseInt(this.compressionX, 10);
        let compressionY = parseInt(this.compressionY, 10);

        if (compressionX < 4 || compressionX > 20) return;
        if (compressionY < 4 || compressionY > 20) return;

        compressionX = this.compressionX;
        compressionY = this.compressionY;
          ...
          .then((res) => {
            ...
            axios.get('http://localhost:5000/tofe', {
              ...
              .then((tofe) => {
                ...
                [this.xMin, this.yMin] = [bytes[0], bytes[1]];
                const [xBins, yBins] = [bytes[2], bytes[3]];
                bytes[0] = 0;
                bytes[1] = 0;
                bytes[2] = 0;
                bytes[3] = 0;
                ...
                // reconstruct ToF-E data array
                for (let x = 0; x < xBins; x++) {
                  const row = [];
                  for (let y = 0; y < yBins; y++) {
                    let events = bytes[byte++]; // goes through array one Uint16 at a time
                    if (events > 0) { // log2(0) == -Infinity so this check is needed
                      if (events === 1) {
                        events = 1.5; // log2(1) == 0 so we increase ones to 1.5
                      }
                      row.push(Math.log2(events));
                    } else {
                      row.push(0);
                    }
                  }
